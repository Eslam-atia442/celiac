...
class RateController extends BaseApiController
{
    ...
    public function store(RateRequest $request): JsonResponse
    {
        try {
            ...
            if(!$rate)
                return $this->respondWithError(__('messages.validation.your_rate_already_exist'));
     ...
     * @return JsonResponse
     */
    public function update(RateRequest $request, Rate $evaluation): JsonResponse
    {
        try {
            $evaluation = $this->service->update($evaluation, $request->validated());
            return $this->respondWithModel($evaluation->load($this->relations));
        } catch (Exception $e) {
            return $this->respondWithError($e->getMessage());
        }
    }
